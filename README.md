    ...
    <style>
        * {
        ...
        body {
            ...
            background-image: url('assets/aerial-view-container-cargo-ship-sea.jpg');
    ...
    <script>
        ...
        document.getElementById('survey-form').addEventListener('submit', async function(event) {
            ...
            const formData = {
                ...
                dgStatus: document.querySelector('input[name="dg-status"]:checked')?.value || ''
            };

            // Kumpulkan data container dalam array
            const containers = [];
            for (let i = 1; i <= containerCount; i++) {
                const containerNumber = document.getElementById(`container-number-${i}`)?.value || '';
                const containerType = document.getElementById(`container-type-${i}`)?.value || '';
                const setTemp = document.getElementById(`set-temp-${i}`)?.value || '';
            ...
            try {
                const response = await fetch('https://script.google.com/macros/s/AKfycbwNGVnfGwrsutr6l8S07AEoYYqB_gVmt3_GO3Ysnqw8FfGMsj7LIhvOwfilewMvOBEI/exec', {
                    method: 'POST',
                    body: JSON.stringify(formData),
                    headers: { 'Content-Type': 'application/json' },
                    mode: 'no-cors' // Tambahkan mode no-cors untuk menghindari masalah CORS
                });
                // Karena mode no-cors, kita tidak bisa membaca respons langsung
                console.log('Data dikirim, periksa spreadsheet Anda.');
